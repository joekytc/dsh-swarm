        ...
        ? async (prompt, workspaceDir) => {
            const h = await agents.create({
                ...
                setup: async (agentCtx) => {
                    ...
                    toolsSvc?.guard?.((e) => buildReadOnlyWriteGuard(repoRoot)(e));
                },
            });
    ...
    registry.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            if (!pctx || !pctx.checklist || !pctx.checklistRef) {
                return { kind: 'openspec', approved: false, reason: 'no-checklist', guidance: '尚未保存需求澄清清单：请先调 planning_prefetch 采集仓库事实、完成 grill-me 澄清后，调 planning_checklist_save 保存清单，再发 /openspec: 确认。' + MATTPOCOCK_PLANNING_GUIDANCE };
            }
            const input = { workspaceDir: pctx.workspaceDir, checklist: pctx.checklist, checklistRef: pctx.checklistRef };
