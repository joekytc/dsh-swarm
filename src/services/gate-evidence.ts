...
export async function writeGateLog(dir: string, taskId: string, report: GateRunReport): Promise<string> {
  const logDir = join(dir, 'gate-logs');
  await mkdir(logDir, { recursive: true });
  const path = join(logDir, `${taskId}.log`);
  const lines: string[] = [`== ${new Date().toISOString()} ok=${report.ok}`];
  ...
  if (report.failure) lines.push(`-- failure: ${report.failure.code}: ${report.failure.detail}`);
