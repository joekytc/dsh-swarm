...
export function sessionPresetOf(ctx, agent) {
    ...
    if (a?.ctx !== undefined && typeof presets?.composedPreset === 'function') {
        let composed;
        try {
            composed = presets.composedPreset(a.ctx);
        }
        catch {
            composed = undefined;
        }
    ...
    if (a?.session !== undefined && typeof projections?.stateOf === 'function') {
        let projected;
        try {
            projected = projections.stateOf(a.session, 'agentPreset');
        }
        catch {
            projected = undefined;
        }
