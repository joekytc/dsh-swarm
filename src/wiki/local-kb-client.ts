...
import { WikiError, type WikiSearchResult } from './wiki-vault-client.js';
...
export class LocalWikiClient {
  ...
  private abs(pagePath: string): string {
    assertLocalKbPagePath(pagePath); // 白名单单一来源（wiki/**、拒 .. 与绝对路径），勿内联重复
    const t = resolve(this.root, pagePath);
    if (!isInsideKbRoot(this.root, t)) throw new WikiError('kb-rejected', undefined, `kb-rejected: path escapes local KB root: ${pagePath}`);
    return t;
  }
  ...
  async read(pagePath: string): Promise<{ rawMd: string }> {
    const t = this.abs(pagePath);
    if (!existsSync(t)) throw new WikiError('kb-rejected', undefined, `kb-rejected: page not found: ${pagePath}`);
    try {
  ...
  async search(q: string): Promise<WikiSearchResult[]> {
    ...
    if (!needle) return [];
    const hits: Array<WikiSearchResult & { hits: number }> = [];
