// src/domain/review-target.ts
import type { BoardState, Role, Task, TaskMode } from './types.js';
...
export function resolveReviewTarget(
  ...
  reviewTask: Task,
  chainId: string,
): { currentTarget: Task; root: Task } | null {
  const role: Role = reviewTask.assignee === 'dt' ? 'dt' : 'pt';
  const srcAssignee: Role = role === 'pt' ? 'p' : 'd';
  const srcMode: TaskMode = role === 'pt' ? 'openspec' : 'execute';
  let currentTarget = reviewTask.reworkOfTaskId
    ...
    : reviewTask.parents[0]
      ...
      : undefined;
  if (!currentTarget) {
    currentTarget = [...state.tasks.values()].find(
      (t) => t.chainId === chainId && t.assignee === srcAssignee && t.mode === srcMode && t.status === 'done',
    );
  }
