...
/** 返工卡「本轮修复清单」节（2026-09-15 P0-3）：系统确定性注入上一轮评审 issues 全文——
 *  旧实现 body 逐字拷贝源卡，返工方看不到要修什么 → 全文重写 → 判据面再生（ch_1_mu235xfz 根因 L1/L4）。 */
function reworkChecklistSection(issues: ReviewIssue[] | undefined): string {
  if (!issues || issues.length === 0) return '';
  const unresolved = issues.filter((i) => i.resolved !== true);
  const resolved = issues.filter((i) => i.resolved === true);
  const lines: string[] = [
    '',
    '',
    '## 本轮修复清单（系统确定性注入：上一轮评审未通过项）',
    `上一轮评审共 ${issues.length} 条：未修复 ${unresolved.length} 条、已修复 ${resolved.length} 条。`,
    '本卡为返工轮：只做定点修复，禁止无关联的全文重写与任务编号重排（2026-09-15 收敛决议）。',
  ];
  if (unresolved.length > 0) {
    lines.push('### 未修复（本轮必须处理）');
    for (const i of unresolved) {
      lines.push(`- [${i.severity}] ${i.title}`);
      if (i.location) lines.push(`  - 定位: ${i.location}`);
      if (i.detail) lines.push(`  - 问题: ${i.detail}`);
      if (i.fix) lines.push(`  - 建议改法: ${i.fix}`);
    }
  }
  if (resolved.length > 0) {
    lines.push('### 已修复（不需再改，保持现状）');
    for (const i of resolved) lines.push(`- [${i.severity}] ${i.title}`);
  }
  return lines.join('\n');
}
...
export class KanbanService {
  ...
  async noteImDeliveryFailed(chainId: string, detail: string, actor: Actor): Promise<KanbanEvent> {
    if (actor !== 'system') throw new Error('permission denied: only system may note im delivery failure');
  ...
  async reopenChain(chainId: string, reason: string, actor: Actor): Promise<KanbanEvent> {
    if (!can('reopen-chain', actor, null)) throw new Error('permission denied: only human may reopen chain');
    const chain = await this.chainOf(chainId);
    if (chain.status !== 'blocked') throw new Error('reopenChain requires blocked chain, got: ' + chain.status);
    if (!reason || !reason.trim()) throw new Error('reason required');
    const ev = await this.emit({ chainId, taskId: null, kind: 'chain/reopened', payload: { reason }, author: actor, at: Date.now() });
    const anchor = [...this.state.tasks.values()].filter((t) => t.chainId === chainId).at(-1);
    if (anchor) await this.comment(anchor.id, `[recovery] 人工恢复：链 ${chainId} 由 blocked 恢复为 executing（reason: ${reason}）`, actor);
    return ev;
  }
  ...
  async waiveReview(reviewTaskId: string, targetTaskId: string, reason: string, actor: Actor): Promise<KanbanEvent> {
    if (!can('waive-review', actor, null)) throw new Error('permission denied: only human may waive review');
    const t = this.state.tasks.get(targetTaskId);
    if (!t) throw new Error('unknown target task: ' + targetTaskId);
    if (t.reviewStatus !== 'failed' && t.reviewStatus !== 'gave-up') {
      throw new Error('waiveReview requires target reviewStatus failed|gave-up, got: ' + t.reviewStatus);
    }
    if (!reason || !reason.trim()) throw new Error('reason required');
    const ev = await this.emit({ chainId: t.chainId, taskId: reviewTaskId, kind: 'review/waived', payload: { reviewTaskId, targetTaskId, reason }, author: actor, at: Date.now() });
    await this.comment(targetTaskId, `[recovery] 评审豁免：${reviewTaskId} → ${targetTaskId} 由人工豁免（reason: ${reason}）`, actor);
    return ev;
  }
  ...
  async createReworkTask(input: { sourceTaskId: string; reviewTaskId: string; reason: string; issues?: ReviewIssue[] }, actor: Actor): Promise<Task> {
    ...
    const task: Task = {
      id: nid('t'), chainId: source.chainId, title: `[返工] ${source.title}`, body: source.body + reworkChecklistSection(input.issues),
