...
export function parseIssueEvidence(raw) {
    ...
    if (typeof r['command'] !== 'string' || !r['command'].trim())
        return null;
    if (typeof r['exit'] !== 'number' || !Number.isFinite(r['exit']))
        return null;
...
export function paperCheck(content, claimedExit) {
    if (content === null)
        return 'unreadable';
...
export function isAllowedCommand(command, prefixes) {
    const trimmed = command.trim();
    return prefixes.some((p) => {
        if (!trimmed.startsWith(p))
            return false;
        const rest = trimmed.slice(p.length);
        if (SHELL_META.test(rest))
            return false; // 元字符 = 命令链/替换/重定向，一律拒绝
