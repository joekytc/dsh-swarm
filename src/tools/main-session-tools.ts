  ...
  registry.register(defineTool({
    ...
    async execute(args: { message: string }, exec?: { agent?: { session?: { header?: { cwd?: string } } } }) {
      ...
      if (pctx?.checklist && pctx.checklistRef) {
        ...
        return { kind: 'openspec', chainId: r.chainId, specCardId: r.specCardId, approved: true, firstCard: r.firstCard, guidance: KANBAN_HANDOFF_RULE(configProvider.getEffective().prefixRoutes) } as unknown as JsonValue;
      }
