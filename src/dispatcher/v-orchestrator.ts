...
export const PHASE_INSTRUCTIONS: Partial<Record<VPhase, string>> = {
  ...
  d: [
    ...
    'greenfield（清单 greenfield:true 声明）：TARGET_REPO 取清单 manifest.repo.localPath（无 file-prefetch 附件 ref 时）；body 额外声明 GREENFIELD=1（merge-gate 首合入建基线用）。首步建仓前置：mkdir -p 目标目录 → git init -b main → 写 .gitignore/README → git add + commit 初始提交（tdd 声明用 { skipped: { reason: "greenfield scaffold" } }）→ 再按既有 git worktree/branch → 改代码 → commit → push(feature) 流程。',
