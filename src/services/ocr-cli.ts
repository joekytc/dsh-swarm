...
export const INSTALL_GUIDANCE: string = [
...
export type OcrCliDeps = { execFileFn?: typeof execFile; env?: NodeJS.ProcessEnv; homedirFn?: () => string };
...
function resolveBin(deps: OcrCliDeps = {}): string | null {
...
export async function probeOcr(deps: OcrCliDeps = {}): Promise<{ installed: boolean; version: string; binPath: string | null }> {
  const binPath = resolveBin(deps);
  if (!binPath) return { installed: false, version: '', binPath: null };
  try {
...
export async function runOcr(
  ...
  const binPath = resolveBin(deps);
  if (!binPath) return { stdout: '', stderr: INSTALL_GUIDANCE, error: 'ocr-not-installed' };
  const run = promisify(deps.execFileFn ?? execFile);
  try {
...
export function managedProviderReady(deps: OcrCliDeps = {}): boolean {
  try {
    const home = deps.homedirFn ? deps.homedirFn() : homedir();
    const cfg = JSON.parse(readFileSync(join(home, '.opencodereview', 'config.json'), 'utf8')) as {
      provider?: unknown;
      model?: unknown;
      custom_providers?: Record<string, { model?: unknown } | undefined>;
    };
    if (cfg.provider !== OCR_MANAGED_PROVIDER_NAME) return false;
    const top = typeof cfg.model === 'string' ? cfg.model : '';
    const nested = cfg.custom_providers?.[OCR_MANAGED_PROVIDER_NAME]?.model;
    return (top || (typeof nested === 'string' ? nested : '')).length > 0;
  } catch {
    return false; // 文件不存在/解析失败/字段缺失一律视为未就绪
  }
}

export async function wireManagedProvider(
  a: { baseUrl: string; protocol: 'openai' | 'anthropic'; apiKey: string; model: string },
  deps: OcrCliDeps = {},
  ...
  for (const [name, args] of steps) {
    ...
    if (r.error) return { ok: false, log: `config set ${name} 失败: ${r.error} ${r.stderr.slice(0, 500)}`.trim() };
  }
