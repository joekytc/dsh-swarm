...
import { buildChecklistSlug, KB_PAGE_NAMESPACES_HINT, assertAllowedWikiPagePath } from '../wiki/page-path.js';
  ...
  return [
    defineTool({
      ...
      async execute(args: { checklist: unknown; restoreRef?: string }) {
        ...
        const repoSlug = buildRepoSlug(checklist.manifest.repo.localPath);
        const pagePath = `${pagePrefix}${repoSlug}/checklists/${buildChecklistSlug(checklist.requirementName ?? checklist.spec.problem)}-${Date.now().toString(36)}.md`;
        try {
    ...
    defineTool({
      ...
      description: 'Save a distilled learning (experience) to the knowledge base. scope=chain → projects/<repoSlug>/<chainId>/learnings/ (requirement-level); scope=project → projects/<repoSlug>/learnings/ (repo-level). repoSlug is derived from the chain workspaceDir; both scopes require chain.workspaceDir. Returns ref. Soft-fails {ok:false,reason:"kb-unreachable"} when KB is unreachable (no temp fallback).',
      parameters: {
      ...
      async execute(args: { learning: unknown; scope: 'chain' | 'project'; chainId: string }) {
        ...
        const wsRoot = `projects/${buildRepoSlug(chain.workspaceDir)}/`;
        const prefix = args.scope === 'chain' ? `${wsRoot}${args.chainId}/learnings/` : `${wsRoot}learnings/`;
    ...
    defineTool({
      ...
      description: 'Recall KB memory for planning. path mode: read a full page (whitelist: ' + KB_PAGE_NAMESPACES_HINT + ') truncated to 8000 chars. query mode: full-text search returning top 5 {path,title,score}. Returns {ok:false,reason:"kb-unreachable"} on KB failure; {ok:false,reason:"disabled"} when memory is disabled.',
      parameters: {
