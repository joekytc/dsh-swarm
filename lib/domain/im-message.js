...
export function parseChainReport(raw) {
    ...
    if (!requirement || !status || !branch || !acceptance || !verification)
        return null;
    ...
    for (const entry of r['tasks']) {
        if (typeof entry !== 'object' || entry === null)
            return null;
        const text = reportText(entry['text']);
...
function renderDeliveryReport(chain, chainId, report, wHandoff, attention) {
    const lines = [];
    ...
    if (report.leftovers.length > 0) {
        lines.push('遗留（非阻塞）：');
        for (const l of report.leftovers)
            lines.push(`- ${l}`);
    }
...
export function buildCompletionMessage(state, chainId, completedTaskId, now) {
    ...
    if (report)
        return renderDeliveryReport(chain, chainId, report, wHandoff, attention);
    const lines = [];
