...
export const GIT_PAT_ENV_NAMES = ['KANBAN_GIT_PAT', 'GIT_PAT', 'GH_TOKEN', 'GITHUB_TOKEN'] as const;

/** 从进程 env 解析非空 PAT；无则返回 null。 */
export function resolveGitPat(env: NodeJS.ProcessEnv): string | null {
...
export async function resolveGitPatFromCtx(ctx: unknown, env: NodeJS.ProcessEnv): Promise<string | null> {
  const creds = (ctx as { get?(n: string): unknown }).get?.('credentials') as
...
export function gitAuthHeader(pat: string): string {
 ...
 */
export function injectGitCredentials(repoDir: string, pat: string): { ok: boolean; detail: string } {
  const targets: string[] = [];
  try {
    const url = execFileSync('git', ['-C', repoDir, 'remote', 'get-url', 'origin'], { encoding: 'utf8' }).trim();
    if (url.startsWith('http://') || url.startsWith('https://')) targets.push(url);
  } catch {
    // 非 git 仓库或无 origin：注入目标为空
  }
  if (targets.length === 0) return { ok: false, detail: 'no http remote origin' };
  try {
