...
export const PHASE_INSTRUCTIONS: Partial<Record<VPhase, string>> = {
  p: [
    ...
    'greenfield（清单 greenfield:true）：openspec 产物仍写目标目录 `openspec/changes/<change_name>/`（目录可先不存在，空目录可写）；P 不 git init——建仓属 D 职责，P 保持只读。',
    'tasks.md 拆分粒度（结构准备度，与 PT 同一判据）：每个任务条目必须有单一、可独立核对的完成判据；禁止一条任务/一条测试用例混 N≥2 个互不依赖的可观察结果——按可独立验证的行为逐条拆分。完成判据必须是实施完成后的单一终态；不写执行层阶段态判据（RED/GREEN/REFACTOR、"先失败再通过"属 D 阶段执行协议，见 D 阶段模板）。',
    'proposal.md 必须含「上游协议遵循说明」节：逐条列规格卡/澄清清单声明的工程协议引用（可定位出处：路径/明确协议名）+ 计划如何遵守；未声明任何协议则该节写「无」。禁止引用规格卡之外的协议标准。',
    '需求对齐（颗粒度）：澄清清单/规格卡的任务项、任务点、注意点 ↔ 计划任务逐条双向映射——不遗漏；超纲任务须能溯源到澄清回答。',
    '逻辑交互一致性：计划内部不得自相矛盾、依赖顺序可执行；impl_decisions 多方案必须在计划内收敛为唯一路径，不留未定分支。',
    '返工轮约束（2026-09-15）：任务体含「本轮修复清单」节时，沿用既有任务编号、只做清单内条目的定点修复；禁止重排编号与清单无关的全文重写/扩写。',
  ...
  pt: [
    '## PT 阶段任务体要求（计划评审，只读）',
    'body 写入计划评审指令：P 已判定需要计划评审（理由见上）。只读评审 P 的计划产物，按五要素核对：需求对齐（澄清清单/规格卡条目↔任务双向映射，不遗漏不超纲）/完整性/逻辑交互一致性/结构准备度（每任务条目单一、可独立核对的完成判据；混 N≥2 个互不依赖的可观察结果=混行为须拆分；看计划文本自身，不要求执行层字样）/工程协议一致性（只对账 proposal「上游协议遵循说明」节，未声明协议不得自行引入；TDD/RED-GREEN 属 D 阶段内置协议，不在对账范围）。输出 verdict+issues 入交接 metadata.review_evidence。',
    '评审源正相关（2026-09-15）：评审源 = 规格卡功能面 + 计划文本；环境/工具/凭证/权限/接口来源等需求外事项不得作为阻断性判据（有价值者置非阻塞建议，由执行环节验证）。',
    'issues 四要素缺一无效：location（哪条任务/哪节）+ 依据（上游声明引用或计划内部矛盾点）+ 问题 + 可执行建议；pass 时 issues 只放非阻塞建议。',
    '返工复审：body 含「上一轮评审未通过 issues」节时必须先逐条三态对账（已修复/未修复/部分修复），未修复旧 issue 原样沿用，再提新问题。对账条目必须写 legacy=true、本轮新问题 legacy=false/省略——收敛闸据此判定：旧账未清继续返工；旧账清零后仅未解决 critical 可继续 fail，其余降级为下游「评审遗留建议」。不要在旧账已修复的同一轮用更细的新维度继续 fail（不在五要素内的细化不算阻断）。',
  ...
  d: [
    ...
    'greenfield（清单 greenfield:true 声明）：TARGET_REPO 取清单 manifest.repo.localPath（无 file-prefetch 附件 ref 时）；body 额外声明 GREENFIELD=1（merge-gate 首合入建基线用）。首步建仓前置：mkdir -p 目标目录 → git init -b main → 写 .gitignore/README → git add + commit 初始提交（tdd 声明用 { skipped: { reason: "greenfield scaffold" } }）→ 再按既有 git worktree/branch → 改代码 → commit → push(feature) 流程。；若目标目录已有 .git（声明与事实不符）→ 跳过建仓前置，直接走既有 worktree 流程，并在 complete 摘要注明声明与事实不符',
...
function extractPtPassSuggestions(state: BoardState, chainId: string): ReviewEvidence['issues'] {
  ...
  if (!Array.isArray(ev?.issues)) return [];
  // 收敛闸降级通过：issues 含已对账（resolved=true）条目 → 只把未解决项传给下游；
  // 真 pass 保持全量（pass 时 issues 即非阻塞建议留档）。
  return ev!.downgraded ? ev!.issues.filter((i) => i.resolved !== true) : ev!.issues;
}
...
export class VOrchestrator {
  ...
  private async wakeVInner(chainId: string): Promise<void> {
    ...
    // 2026-09-15 恢复能力：人工恢复（chain/reopened）后重置 V 内建 stall 计数——每个恢复事件只消费一次
    //（不可每轮无条件清零，否则 stall 计数失效、[create-failed] 永不触发）。给足重试额度，避免因历史
    // stallCount 残留立即再撞上限（LRN-20260902-002：事件永不再来=永久停滞）。
    const lastReopen = [...state.events].reverse().find((e) => e.kind === 'chain/reopened' && e.chainId === chainId);
    if (lastReopen && lastReopen.seq > (orch.reopenConsumedSeq ?? -1)) {
      orch.stallCount = 0;
      orch.reopenConsumedSeq = lastReopen.seq;
    }
  ...
  private async handleReviewCompletion(
    ...
    // 幂等：该评审卡已有 review 终态事件（重启恢复 / 人工恢复后重入）→ 按**最后一条** review 事件判定：
    // passed / waived → 推进；failed / gave-up → 保持（等返工链或人工裁决）。
    // 2026-09-15 修正（ch_1_mu235xfz 死结第二节）：旧实现用布尔 OR，gave-up 恒落 'rework' →
    // 人工豁免/恢复后重入仍走不出去（该分支在 getVAgent 之前，纯空转零事件）。
    const lastReview = [...state.events]
      .reverse()
      .find((e) => e.taskId === reviewTask.id
        && (e.kind === 'review/passed' || e.kind === 'review/failed' || e.kind === 'review/gave-up' || e.kind === 'review/waived'));
    if (lastReview) {
      return lastReview.kind === 'review/passed' || lastReview.kind === 'review/waived' ? 'advanced' : 'rework';
    }
    ...
    // fail
    await this.kanban.recordReview(reviewTask.id, root.id, evidence, 'system');
    // 收敛闸（2026-09-15 Q7/Q10，治本）：旧账清零后，仅未解决的 critical 允许继续返工；其余降级为
    // 下游「评审遗留建议」并按通过处理 —— 保证必然收敛。ch_1_mu235xfz 教训：4 轮对账 12 条全清仍 fail
    //（"计划自述类"新问题可在返工文本上无限再生），唯一出口是 gave-up 熔断。
    const unresolved = evidence.issues.filter((i) => i.resolved !== true);
    const legacyUnresolved = unresolved.filter((i) => i.legacy === true); // 漏标=false=视为新问题（方向安全）
    // 生效范围（2026-09-15 修正）：仅 PT 计划评审 + 复审轮（reviewAttempt ≥ 1）。
    // 首轮评审保持原语义（high 阻断，给计划 1 轮返工机会，符合"小需求 1 轮返工通过"预期）；
    // DT 评审依据机械实证（test/build/lint/diff），severity 门槛会误放行真问题 → 不适用。
    const isPtReworkRound = role === 'pt' && (currentTarget.reviewAttempt ?? 0) >= 1;
    if (isPtReworkRound && legacyUnresolved.length === 0 && !unresolved.some((i) => i.severity === 'critical')) {
      await this.kanban.comment(reviewTask.id, [
        `[review-final] 收敛闸：旧账已清零（0 条未修复遗留），本轮 ${unresolved.length} 条新问题无 critical → 降级为下游「评审遗留建议」，本轮评审按通过处理。`,
        '## 降级条目（原文）',
        formatIssues(unresolved) || '  -（无）',
      ].join('\n'), 'system');
      await this.kanban.recordReview(reviewTask.id, root.id, {
        ...evidence,
        verdict: 'pass',
        downgraded: { from: 'fail', reason: '旧账清零且无未解决 critical（收敛闸）' },
      }, 'system');
      return 'advanced';
    }
    ...
    const rework = await this.kanban.createReworkTask({
      sourceTaskId: currentTarget.id,
      reviewTaskId: reviewTask.id,
      reason: 'review failed',
      issues: evidence.issues, // 2026-09-15 P0-3：issues 全文随卡传递（返工卡 body「本轮修复清单」节）
    }, 'system');
