...
export function validateReviewEvidence(role: 'pt' | 'dt', handoff: Handoff | undefined): string[] {
  ...
  // 2026-09-15 口径锁死（P1-7，仅 PT）：fail 必须至少一条未解决的 critical/high——
  // persona 口径「fail = 存在 critical/high」机械落地，防 medium 冒充阻断（ch_1_mu235xfz R3 summary 实录）。
  // 全为非阻断项时应输出 pass（issues 作为非阻塞建议留档）。DT 不适用：其实证失败（test/build/diff）
  // 未必伴随 high issue，用 severity 门槛会误放行真问题。
  if (role === 'pt' && ev.verdict === 'fail' && Array.isArray(ev.issues)) {
    const blocking = ev.issues.some((i) => i.resolved !== true && (i.severity === 'critical' || i.severity === 'high'));
    if (!blocking) missing.push('review_evidence.issues (verdict=fail 需至少一条未解决的 critical/high；否则应判 pass)');
  }

  if (role === 'pt') {
    // 计划结构字段：评审须给出被评审计划产物引用（如 artifacts_path/page 结构）——缺则视为证据不足
    const planRef = ev.reviewPage ?? (handoff.metadata ?? {})['artifacts_path'];
    if (!planRef) missing.push('metadata.artifacts_path (被评审计划的 openspec 目录绝对路径，继承被评审 P 卡) 或 review_evidence.reviewPage');
  } else {
