...
const DOC_CONFIG_EXT = new Set(['md', 'txt', 'json', 'yml', 'yaml', 'toml']);
function extOf(path) {
...
export function classifySkippedDeclaration(diffFiles) {
    if (diffFiles === null)
        return { action: 'allow-alarm', reason: 'tdd skipped by declaration (diff unavailable, 保守放行)' };
...
export function classifyTestFilesDeclaration(diffFiles) {
    if (diffFiles === null)
...
const isSafeRelativeTestFile = (f) => /^[A-Za-z0-9._/-]+$/.test(f) && !isAbsolute(f) && !f.split(/[/\\]/).includes('..')
...
export function deriveGatePlan(input) {
    ...
    if (!input.config.enabled)
        return { kind: 'silent-skip', reason: 'gates disabled' };
    ...
    const files = tdd.test_files.filter((f) => typeof f === 'string' && f.trim().length > 0);
    if (files.length === 0)
    ...
    if (declared.action === 'bounce')
        return { kind: 'bounce', reason: declared.reason };
