...
function wireWhenAvailable(ctx: Context, name: string, fn: () => void, timeoutMs = 60000): () => void {
  if (ctx.get(name)) { fn(); return () => {}; }
  const timer = setInterval(() => {
    if (ctx.get(name)) { clearInterval(timer); clearTimeout(to); fn(); }
  }, 500);
