...
//   PRD 原文切片  projects/<repoSlug>/source-docs/<slug>-part-NN.md  （planning_prd_collect，代码强制）
...
export const KB_PAGE_NAMESPACES_HINT = 'projects/<repoSlug>/checklists/、projects/<repoSlug>/source-docs/、projects/<repoSlug>/learnings/、projects/<repoSlug>/ch_*/learnings/、projects/<repoSlug>/ch_*/t_*.md、projects/<repoSlug>/ch_*/review/、projects/<repoSlug>/reviews/<topic>-<ymd>/';
/** 白名单：projects/<repoSlug>/ 下六类命名空间（repoSlug=[a-z0-9-]+，杜绝 LLM 自造路径）。 */
const KB_PAGE_PATH_RE = /^projects\/[a-z0-9-]+\/(?:checklists\/|learnings\/|source-docs\/|ch_[0-9a-z_]+\/(?:t_[0-9a-z_]+\.md|review\/|learnings\/))/;
...
export function assertAllowedWikiPagePath(pagePath) {
    if (!isAllowedWikiPagePath(pagePath)) {
        throw new WikiError('kb-rejected', undefined, `kb-rejected: page path outside allowed namespaces (${KB_PAGE_NAMESPACES_HINT}): ${pagePath}`);
    }
}
// ── 本地 KB（llm-wiki）路径规则（双模式）──────────────────────────────
export const LOCAL_CHECKLIST_PREFIX = 'wiki/queries/checklists/';
export const LOCAL_SOURCE_DOCS_PREFIX = 'wiki/queries/source-docs/';
