            ...
            const setup = async (agentCtx) => {
                ...
                if (isDExecute && dRepo) {
                    ...
                    if (pat) {
                        ...
                        if (cred.ok)
                            console.error('[dsh-kanban][debug] git credential injected task=' + taskId + ' targets=' + cred.detail);
                        else
                            console.error('[dsh-kanban][debug] git credential inject skipped task=' + taskId + ' repo=' + dRepo + ': ' + cred.detail);
                    }
                }
            };
            try {
                ...
                if (candidates.length === 0) {
                    ...
                    agent = hasRunHistory
                        ? (await this.ctx.get('agents').resume({
