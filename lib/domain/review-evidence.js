...
export function validateReviewEvidence(role, handoff) {
    ...
    if (role === 'pt' && ev.verdict === 'fail' && Array.isArray(ev.issues)) {
        ...
        if (!blocking)
            missing.push('review_evidence.issues (verdict=fail 需至少一条未解决的 critical/high；否则应判 pass)');
    }
    if (role === 'pt') {
        // 计划结构字段：评审须给出被评审计划产物引用（如 artifacts_path/page 结构）——缺则视为证据不足
        const planRef = ev.reviewPage ?? (handoff.metadata ?? {})['artifacts_path'];
        if (!planRef)
            missing.push('metadata.artifacts_path (被评审计划的 openspec 目录绝对路径，继承被评审 P 卡) 或 review_evidence.reviewPage');
    }
    else {
