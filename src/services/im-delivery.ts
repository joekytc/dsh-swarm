...
function idTokens(o: BotAskOption): string[] {
  const out: string[] = [normAnswer(o.botId)];
  if (o.label) {
...
export function matchBotAnswer(options: BotAskOption[], raw: string): string {
  const target = normAnswer(raw);
  if (!target) return '';
  const exact = options.find((o) => idTokens(o).includes(target));
  if (exact) return exact.botId;
