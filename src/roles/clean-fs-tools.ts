...
export const ESCALATION_PARAM_KEYS = ['sandbox_permissions', 'justification'] as const;

/** 需去毒的宿主工具名单：write/edit（dsh-tool-fs）+ bash（dsh-tool-bash，schema 同样带毒）。 */
const CLEAN_FS_TOOL_NAMES = ['write', 'edit', 'bash'] as const;
...
/** 宿主工具定义的最小结构面（dsh-tools ToolDefinition 的本模块消费子集）。 */
...
type MaybeShadowedDefinition = HostToolDefinition & { [CLEAN_FS_BRAND]?: boolean };

/** dsh-tools ToolRuntime 的本模块消费子集（get/register，与 agent-runner resumeOrReuse 同一访问面）。 */
...
function toolsRuntimeOf(agentCtx: Context): ToolsRuntimeLike | undefined {
  const tools = (agentCtx as unknown as { tools?: Partial<ToolsRuntimeLike> }).tools;
  if (!tools || typeof tools.get !== 'function' || typeof tools.register !== 'function') return undefined;
  return tools as ToolsRuntimeLike;
}
 ...
 */
export function installCleanFsTools(agentCtx: Context, agent: unknown): void {
  const tools = toolsRuntimeOf(agentCtx);
  if (!tools) return; // 无工具服务（测试桩）跳过；宿主版本自然保持可见
  for (const name of CLEAN_FS_TOOL_NAMES) {
    try {
      ...
      const visible = tools.get(name, agent) as MaybeShadowedDefinition | undefined;
      ...
      if (visible[CLEAN_FS_BRAND]) continue; // 幂等：上次 setup 已挂 shadow
      const props = (visible.parameters as { properties?: Record<string, unknown> }).properties;
      const poisoned = !!props && ESCALATION_PARAM_KEYS.some((k) => props[k] !== undefined);
      if (!poisoned) continue; // 宿主 schema 本就干净（无沙箱组合）→ 无需 shadow
      ...
      const parameters = JSON.parse(JSON.stringify(visible.parameters)) as Record<string, unknown>;
      if (parameters['properties'] && typeof parameters['properties'] === 'object') {
        for (const key of ESCALATION_PARAM_KEYS) delete (parameters['properties'] as Record<string, unknown>)[key];
      }
      // 防御：宿主未来若把两参数标成必填，required 一并过滤（当前为可选，此过滤为空操作）
      if (Array.isArray(parameters['required'])) {
        parameters['required'] = (parameters['required'] as unknown[]).filter(
          (k) => !ESCALATION_PARAM_KEYS.includes(k as (typeof ESCALATION_PARAM_KEYS)[number]),
        );
      }
      tools.register({
        ...visible,
        ...
        execute(args: unknown, exec: unknown) {
          return (visible.execute as (a: unknown, e: unknown) => Promise<unknown>)(stripEscalationArgs(args), exec);
        },
      });
    } catch (err) {
