...
describe('WikiVaultClient getter 热生效', () => {
  it('构造后改写 getCfg 返回新 baseUrl（无需重建实例）', () => {
    let cfg = { baseUrl: 'http://a', pagePrefix: 'projects/' };
    const c = new WikiVaultClient(() => cfg);
    expect(c.baseUrl).toBe('http://a');
    cfg = { baseUrl: 'http://b', pagePrefix: 'projects/' };
    expect(c.baseUrl).toBe('http://b');
  });
});
