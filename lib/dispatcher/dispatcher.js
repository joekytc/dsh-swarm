...
import { sessionPresetOf } from './session-preset.js';
...
export class Dispatcher {
    ...
    stateFile;
    logFile;
    agents; // 宿主 agents 注册表（热重载豁免判据）
    stallProbe;
    lastSeq = null; // null=尚未加载（首轮 tick 从状态文件/事件日志尾行恢复）
    orphanReconciled = false; // 启动 reconcile 一次性闸：仅首轮 tick 执行孤儿收敛
    inFlight = false;
    stallState = new Map();
    timer = null;
    constructor(deps) {
...
function startDispatcherInner(ctx, configProvider, storageDir, logFile, provider, agents) {
    ...
    const auditor = new ChainAuditor({
        ...
        // preset 判定统一走 session-preset.ts（引擎真相优先）：只读 header 会把
        // 「蜂群模式」标签会话（header=宿主默认 ptc）误判成主会话越权。
        readPreset: (agent) => sessionPresetOf(ctx, agent),
    });
    ...
    // 三个 preset 判定 guard 共用统一读面（session-preset.ts）：引擎真相优先，避免
    // 「蜂群模式」标签会话（header=宿主默认 ptc）绕过只读/评审硬闸。
    const readPreset = (agent) => sessionPresetOf(ctx, agent);
    const unguardSubagents = toolsSvc?.guard?.(buildSubagentTreeGuard({ readPreset }));
    if (unguardSubagents) {
        ctx.on('dispose', unguardSubagents);
    }
    // 蜂群模式硬闸：会话 preset==='swarm'（统一读面，非 header）时只读+git 反选拦截；
    // 内部对其余会话恒放行。dispose 时注销（与 subagent guard 同款）。
    const unguardSwarm = toolsSvc?.guard?.(buildSwarmSessionGuard({ readPreset }));
    if (unguardSwarm) {
        ctx.on('dispose', unguardSwarm);
    }
    // 独立评审硬闸：会话 preset==='kanban-dt'（统一读面）且未经角色组合（session id 非 kbn-
    ...
    if (unguardStandalone) {
        ctx.on('dispose', unguardStandalone);
    }
