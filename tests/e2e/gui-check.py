...
宽度 ≤660px+2px、全高（中心栏可视高度）、多链路、详情往返、SSE 订阅、断线 banner、深浅主题、
...
def assert_tab_layout(page) -> None:
    """布局约束：无 .dsh-kb-panel/.dsh-kb-resize；宽度 ≤ 660px+2px；高度 = 中心栏可视高度。"""
    ...
    assert rects["width"] <= 660 + 2, f"tab width {rects['width']:.1f} exceeds 660px+2px"
...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        for vw, vh in [(900, 800), (1200, 800), (1600, 900)]:
            ...
            print(f"viewport {vw}x{vh} tab ≤660px full-height detail roundtrip: OK")
            page.close()
