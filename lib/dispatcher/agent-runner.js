...
function logToDispatcherLog(configProvider, msg) {
    try {
 ...
 *  返回 false。 */
export function isRoleComposed(agent) {
    return !!agent && typeof agent === 'object' && roleCompositions.has(agent);
}
...
export class AgentRunner {
    ...
    defaultModel;
    probeOcrFn;
    constructor(ctx, kanban, configProvider, wiki, defaultModel, deps) {
        this.ctx = ctx;
        this.kanban = kanban;
...
${task.body}`);
        ...
        if (task.assignee === 'dt') {
            const re = this.configProvider.getEffective().reviewEngine;
            const mode = re?.mode ?? 'delegate';
            const provider = re?.managed?.provider ?? '';
            const model = re?.managed?.model ?? '';
            parts.push('## 评审引擎（配置事实，唯一权威；禁止从历史交接推断）', `- reviewEngine.mode = ${mode}${provider && model ? `（managed: provider=${provider} model=${model}）` : ''}`, mode === 'managed'
                ...
                : "- 委托：ocr_review{sub:'preview'} → ocr_review{sub:'rule'} → 自行 git diff 逐文件评审", '- 历史 handoff / 旧链交接里的「托管未配置 / 回落委托」等叙述属过期事实，不得当现状。');
        }
        ...
        if (parents.length > 0) {
            ...
            for (const h of parents) {
                ...
                parts.push(`- metadata: ${JSON.stringify(slimParentMetadata((h.metadata ?? {})))}`);
            }
        }
        ...
        if (task.reworkOfTaskId && task.reviewStatus === 'pending') {
            ...
            for (;;) {
                ...
                if (!up?.reworkOfTaskId || seenIds.has(up.reworkOfTaskId))
                    break;
