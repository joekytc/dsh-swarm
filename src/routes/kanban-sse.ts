...
export function writeSseEvent(res: ServerResponse, event: KanbanEvent): void {
  res.write(`id: ${event.seq}\ndata: ${JSON.stringify(event)}\n\n`);
}

function lastSeen(req: IncomingMessage): number {
...
export async function serveKanbanEvents(
  req: IncomingMessage,
  res: ServerResponse,
  service: Pick<KanbanService, 'subscribe' | 'eventsSince'>,
  ...
  let replaying = true;
  const buffered: KanbanEvent[] = [];
  let heartbeat: ReturnType<typeof setInterval> | undefined;
  const stop = service.subscribe((event) => {
    if (replaying) buffered.push(event);
    else if (event.seq > cursor) { writeSseEvent(res, event); cursor = event.seq; }
  });
  let closed = false;
  const close = () => {
    if (closed) return;
  ...
  for (const event of await service.eventsSince(cursor + 1)) {
    if (event.seq > cursor) { writeSseEvent(res, event); cursor = event.seq; }
  }
  replaying = false;
