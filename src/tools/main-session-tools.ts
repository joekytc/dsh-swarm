...
import { extractChecklistJson, type PlanningChecklist } from '../domain/planning-checklist.js';
...
export function registerMainSessionTools(ctx: Context, configProvider: ConfigProvider): void {
  ...
  registry.register(defineTool({
    ...
    async execute(args: { message: string; intent?: string }, exec?: { agent?: { session?: SessionLike } }) {
      ...
      const openChainFromContext = async (pctx: PlanningContext & { checklist: PlanningChecklist; checklistRef: string }): Promise<JsonValue> => {
      ...
      let machineRestored: { path: string; checklist: PlanningChecklist } | null = null;
      for (const cpath of candidates) {
        try {
          const d = await wiki.read(cpath);
          const c = extractChecklistJson(d.rawMd);
          if (c) { machineRestored = { path: cpath, checklist: c }; break; }
        } catch { /* 单页读取失败 → 试下一候选 */ }
      }
      if (machineRestored) {
        const cur = planningBySession.get('session_main') ?? { workspaceDir: null, sessionId: 'session_main', checklist: null, checklistRef: null, checklistSource: null, requirementName: null };
        const rpctx: PlanningContext & { checklist: PlanningChecklist; checklistRef: string } = { ...cur, checklist: machineRestored.checklist, checklistRef: machineRestored.path, checklistSource: 'kb', requirementName: machineRestored.checklist.requirementName ?? null };
