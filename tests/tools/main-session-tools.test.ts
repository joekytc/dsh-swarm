...
describe('buildSpawnPrdCollect (采集子代理缝)', () => {
  it('buildSpawnPrdCollect: 白名单缝启动 + 技能目录枚举进 prompt', async () => {
    ...
    // skill 必须在列——枚举进 prompt 的采集技能（如 huashu-chrome）需经 skill 工具加载正文才可用
    expect(req.toolFilter).toEqual({ allow: ['web_fetch', 'read', 'glob', 'grep', 'skill'] });
  });
  ...
  it('buildSpawnPrdCollect: 无 workspaceDir → fail-fast（不回退 process.cwd()）', async () => {
    const start = vi.fn();
    const ctx = { get: (k: string) => (k === 'subagents' ? { start } : undefined) } as unknown as Context;
    const spawn = buildSpawnPrdCollect(ctx)!;
    await expect(spawn('p', '', PARENT)).rejects.toThrow(/missing workspace/);
