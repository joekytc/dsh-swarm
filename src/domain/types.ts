...
// 'execute'：D 阶段=执行者（实际写代码/git 提交推送）；'align' 保留兼容旧链路（只读对齐/校验语义已废弃）。
...
export type EventKind =
  ...
  | 'chain/title-updated' // 链标题改名（GUI human only）
  | 'chain/im-delivery-failed' // IM 投递失败留痕（仅 system 机械记账；非状态转换，projection no-op）
...
export interface BoardState {
  ...
  auditWarnings: Map<string, ChainAudit>; // chainId → 验收核对视图
  events: KanbanEvent[];
}
