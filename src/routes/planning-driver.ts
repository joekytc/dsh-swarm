...
export function buildPlanningContext(chainId: string, card: SpecCard, attachments: SpecCardAttachment[]): string {
  return [
...
export async function approveIfReady(
  ...
  if (parsed.kind !== 'openspec') return { ok: false, missing: ['prefix'], guidance: MATTPOCOCK_PLANNING_GUIDANCE };
  const state = await service.snapshot();
  const card = state.specCards.get(specCardId);
  if (!card) return { ok: false, missing: ['spec-card'], guidance: MATTPOCOCK_PLANNING_GUIDANCE };
