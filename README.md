...
- 内容宽度 715-780px（最小 715px / 最大 780px），全高布局；无浮层、无拖拽、无宽度记忆
...
组件层已验证（Vitest）：SSE 补偿/去重、board store 缺口重拉、多链路轨道、五区详情、tab 布局（715-780px 全高）、HTTP action 校验全部通过；真实 GUI 验收请在有 dsh web（3080）运行的环境执行 `gui-check.py`，不要在已有 dsh web 服务时再起第二个实例。
