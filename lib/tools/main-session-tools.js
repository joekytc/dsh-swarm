...
export function registerMainSessionTools(ctx, configProvider) {
    ...
    const wiki = ctx.get('wiki') ?? new WikiVaultClient(() => configProvider.getEffective().wikiVault);
    ...
    registry.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            if (pctx?.checklist && pctx.checklistRef) {
                ...
                return { kind: 'openspec', chainId: r.chainId, specCardId: r.specCardId, approved: true, guidance: KANBAN_HANDOFF_RULE(configProvider.getEffective().prefixRoutes) };
            }
