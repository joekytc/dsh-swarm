...
const chain = (id: string, status: Chain['status']): Chain => ({
  id, title: id === 'ch_running' ? '用户登录重构' : id === 'ch_blocked' ? '对话导出失败修复' : '插件发布说明',
...
export function workflowFixture(): BoardState {
  ...
  const events: KanbanEvent[] = [
    ...
    { seq: 7, chainId: 'ch_blocked', taskId: 't_blocked', kind: 'task/blocked', payload: { reason: 'kb-unreachable' }, author: 'w', at: 700 },
    { seq: 8, chainId: 'ch_done', taskId: null, kind: 'chain/completed', payload: {}, author: 'system', at: 800 },
  ];
  return {
    chains: new Map([
