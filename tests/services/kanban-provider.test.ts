...
describe('kanban plugin', () => {
  ...
  it('KanbanProvider 构造 (ctx, config, configProvider)：kb_url base 经 getter 读 effective 配置', () => {
    ...
    try {
      ...
      const applyRes = configProvider.applyOverride({ wikiVault: { baseUrl: 'http://hot', pagePrefix: 'projects/' }, roles: { models: {} }, reviewEngine: { mode: 'delegate', managed: { provider: '', model: '' } }, imDelivery: { fallbackBotId: '' } });
