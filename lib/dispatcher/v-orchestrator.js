...
export class VOrchestrator {
    ...
    wiki;
    defaultModel;
    constructor(ctx, kanban, agents, config, orchestrations, wiki, defaultModel) { this.ctx = ctx; this.kanban = kanban; this.agents = agents; this.config = config; this.orchestrations = orchestrations; this.wiki = wiki; this.defaultModel = defaultModel; }
    ...
    async getVAgent(orch) {
        ...
        const spawnWith = async (opts) => {
            ...
            if (!ws)
                throw new Error('workspace-unknown: chain ' + orch.chainId + ' 无 workspaceDir，需重新 /plan:');
