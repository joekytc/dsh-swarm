...
/** 判定文件路径是否位于目标仓库内（含等于）。 */
function isPathInsideRepo(p, repoRoot) {
    const c = p.replace(/\\/g, '/');
...
export function buildReadOnlyWriteGuard(repoRoot) {
    return (execution) => {
        const name = String(execution?.name ?? '');
        const args = execution?.arguments ?? {};
        const strings = [];
        ...
        if (DIRECT_WRITE_TOOLS.has(name) && hitsRepo)
        ...
        if (name === 'bash' || name === 'run_code') {
            ...
            if (cmd && writeRe.test(cmd) && cmd.includes(repoRoot))
                return 'write-to-repo-source-denied: ' + name + ' with write marker targeting repo';
        }
        return undefined;
    };
}
...
export function buildPlanWriteGuard(workspaceRoot) {
    ...
    return (execution) => {
        ...
        if (name === 'bash' || name === 'run_code') {
            ...
            if (GUARD_WRITE_INTENT_RE.test(cmd) && !isPlanCmd(cmd))
                return 'plan-guard: P 写仅允许 openspec/changes/ 目录（禁止改动源码）';
        }
        return undefined;
    };
