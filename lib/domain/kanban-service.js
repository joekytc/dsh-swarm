...
function reworkChecklistSection(issues) {
    ...
    if (unresolved.length > 0) {
        ...
        for (const i of unresolved) {
            ...
            if (i.location)
                lines.push(`  - 定位: ${i.location}`);
            if (i.detail)
                lines.push(`  - 问题: ${i.detail}`);
            if (i.fix)
                lines.push(`  - 建议改法: ${i.fix}`);
        }
    }
...
export class KanbanService {
    ...
    async blockChain(chainId, reason) {
    ...
    async reopenChain(chainId, reason, actor) {
        if (!can('reopen-chain', actor, null))
            throw new Error('permission denied: only human may reopen chain');
        const chain = await this.chainOf(chainId);
        if (chain.status !== 'blocked')
            throw new Error('reopenChain requires blocked chain, got: ' + chain.status);
        ...
        if (anchor)
            await this.comment(anchor.id, `[recovery] 人工恢复：链 ${chainId} 由 blocked 恢复为 executing（reason: ${reason}）`, actor);
        return ev;
    }
    ...
    async waiveReview(reviewTaskId, targetTaskId, reason, actor) {
        if (!can('waive-review', actor, null))
            throw new Error('permission denied: only human may waive review');
        const t = this.state.tasks.get(targetTaskId);
        if (!t)
            throw new Error('unknown target task: ' + targetTaskId);
