 ...
 *  2) GUARD 写动词（touch/mkdir/rm/…）与解释器 -c/-e、原地编辑器 -i 移除——run_code 语境是
 *     真程序代码，不存在裸 shell 动词/解释器单行；真文件写由 CODE_WRITE_RE 覆盖（writeFile/
 *     mkdir/unlink/open 写模式/os./pathlib/shutil），内嵌 shell 重定向由本正则覆盖。 */
  ...
  return (execution) => {
    ...
    if (name === 'bash' || name === 'run_code') {
      ...
      const isWrite = name === 'run_code'
        ? CODE_WRITE_RE.test(cmd) || CODE_REDIRECT_WRITE_RE.test(cmd)
...
export function buildPlanWriteGuard(workspaceRoot: string): (execution: { name?: string; arguments?: unknown }) => string | undefined {
  ...
  return (execution) => {
    ...
    if (name === 'bash' || name === 'run_code') {
      ...
      // 写 API）∪ CODE_REDIRECT_WRITE_RE（带空格重定向）——无空格 > 在 JS 里是运算符（=>、
      // 比较 remaining>0），GUARD_WRITE_INTENT_RE 复用致 openspec 裸 edit 随机误拒（15 次
      // 实证）；GUARD 动词/解释器对 run_code 移除的论证见 CODE_REDIRECT_WRITE_RE 注释。
      const isWrite = name === 'run_code'
        ? CODE_WRITE_RE.test(cmd) || CODE_REDIRECT_WRITE_RE.test(cmd)
