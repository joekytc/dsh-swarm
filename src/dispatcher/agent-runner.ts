...
import type { ReviewIssue, Role, Task } from '../domain/types.js';
...
/** goal 条件启用：spec 卡六段或父交接命中关键词 → 注入目标模式指令。 */
...
export class AgentRunner {
  ...
  private buildContext(task: Task, state: Awaited<ReturnType<KanbanService['snapshot']>>, resume: boolean): string {
    ...
    // 权限提示仅对受限角色显示（v 无写、d/execute 全权、p/w 全权但受工具级写护栏约束——写边界不靠 prompt 软约束）
    ...
    // D(execute) = 唯一执行者（danger-full-access，无权限提示）——目标仓库内实际写代码 + git 提交推送
...
${task.body}`);
    // 评审引擎（配置事实）：reviewEngine.mode 决定 ocr_review 走托管还是委托。此前全链无注入点，
    // 模型只能猜——真实事故（2026-09-23）：面板配 managed 且已 wire，DT 仍按委托跑，并引用旧链
    // 交接里「托管未配置→回落委托」的过期叙述当现状。本节热读配置，是运行时唯一权威来源。
    if (task.assignee === 'dt') {
      const re = this.configProvider.getEffective().reviewEngine as
        | { mode?: 'delegate' | 'managed'; managed?: { provider?: string; model?: string } }
        | undefined;
      const mode = re?.mode ?? 'delegate';
      const provider = re?.managed?.provider ?? '';
      const model = re?.managed?.model ?? '';
      parts.push(
        '## 评审引擎（配置事实，唯一权威；禁止从历史交接推断）',
        `- reviewEngine.mode = ${mode}${provider && model ? `（managed: provider=${provider} model=${model}）` : ''}`,
        mode === 'managed'
          ? "- 托管：调 ocr_review{sub:'managed', from:<TARGET_BRANCH>, to:<branch>} 一次取归一化 findings；仅当工具返回「托管未配置」指引或 status !== 'completed' 时才静默改走委托（preview → rule → git diff 自评）"
          : "- 委托：ocr_review{sub:'preview'} → ocr_review{sub:'rule'} → 自行 git diff 逐文件评审",
        '- 历史 handoff / 旧链交接里的「托管未配置 / 回落委托」等叙述属过期事实，不得当现状。',
      );
    }
    // W local KB 模式：注入本地库根 + llm-wiki 用法 + 排序原则 + 交付约定（remote 模式不注入）。
    // 库内复制/移动补句：DIRECT cp/mv/mkdir 工具永远匹配不到 KB 目标（无 path/file_path
    // 实参）会被护栏拒绝——必须显式指引 W 用 write 工具做库内复制/移动。
    if (task.assignee === 'w' && this.configProvider.mode === 'local') {
      const kbRoot = ensureLocalKbRoot();
      parts.push(
        '## 知识库（本地模式）',
        `- 本地库根：${kbRoot}（llm-wiki 原生结构；只读仓库源码，唯一可写区域为该库根）；库内复制/移动一律用 write 工具（cp/mv 会被护栏拒绝）`,
        '- 经 skill 工具加载 llm-wiki 完成查写（query/ingest/crystallize）；库未初始化时先按其 init 工作流初始化（幂等）',
        '- 检索排序原则：相关性优先 7 : 新鲜度 3',
        '- 交付：page_path = 库根下 wiki/** 相对路径（如 wiki/sources/ch_<chainId>/t_<taskId>.md）；kb_url = 空串',
        '- 本地模式不 ingest 外部 URL；wiki_search/wiki_read/wiki_write 不可用（未注册）',
      );
    }
    // D/DT local 库根注入（审查修订）：d:execute local 文案称「库根见运行时上下文」，
    // 此处必须真的注入，否则 D 不知道去哪读计划；DT fs 写评审页同理。
    if ((task.assignee === 'd' || task.assignee === 'dt') && this.configProvider.mode === 'local') {
      parts.push(
        '## 本地知识库（local KB mode）',
        `- 本地库根：${ensureLocalKbRoot()}`,
        task.assignee === 'd'
    ...
    // 规格卡六段 + 附件注入（经 Chain.specCardId），角色 agent 的输入契约，原汁原味
    ...
    if (resume) {
      // resume 注入运行历史与上次失败原因（不只次数），返工/重试同会话可参考前因
    ...
    // 注入上一轮评审的对账状态 + 未修复项速览。完整修复清单由返工卡 body「本轮修复清单」节承载
    //（单一信息源，2026-09-15 P0-3）——避免同一份 issues 在上下文里重复两遍。
    if (task.reworkOfTaskId && task.reviewStatus === 'pending') {
      // 2026-09-15 P0-1 修正：review/failed 的 targetTaskId 恒为 root（原卡），而 reworkOfTaskId 指向
      // 上一张返工卡 —— 必须沿 reworkOfTaskId 链回溯到 root 再匹配。旧实现第 2 张返工卡起永不命中，
      // 注入 "no review/failed evidence found"，P 盲修（ch_1_mu235xfz 不收敛的直接机制 L1）。
      let rootId = task.reworkOfTaskId;
      const seenIds = new Set<string>([rootId]);
      for (;;) {
        const up = state.tasks.get(rootId);
        if (!up?.reworkOfTaskId || seenIds.has(up.reworkOfTaskId)) break;
        rootId = up.reworkOfTaskId;
        seenIds.add(rootId);
      }
      const reviewFailed = [...state.events]
        .reverse()
        .find((e) => e.kind === 'review/failed' && e.payload['targetTaskId'] === rootId);
      parts.push('## Review guidance (rework task)');
      parts.push('- 上游任务: ' + task.reworkOfTaskId + (rootId !== task.reworkOfTaskId ? `（root: ${rootId}）` : ''));
      if (reviewFailed) {
        const evidence = reviewFailed.payload['evidence'] as { issues?: ReviewIssue[] } | undefined;
  ...
  async runTask(taskId: string): Promise<void> {
    ...
    try {
      ...
      // D(execute)：目标仓库解析（供前置授权判定 + git 凭据注入目标 + 上下文）；
      // 会话 cwd 不再指向仓库（会话必须在主 agent 工作空间）。
      const isDExecute = task.assignee === 'd' && task.mode === 'execute';
      const dRepo = isDExecute ? resolveTargetRepoDir(task, state, sessionCwd) : null;

      // D 目标仓库在会话工作空间外 → 跑 D 前先询问用户是否允许（一次授权，D 以 full-access 执行不再逐次提示）。
      ...
      // resume 判定 = 有运行历史（attempts>0 或存在 claimed 事件）且不是「无会话授权阻塞」的任务；
      ...
      const setup = async (agentCtx: Context): Promise<void> => {
        ...
        // 执行角色（P/W/D）先挂载裁剪 preset（kanban-p/w/d），把 shell/approval 等服务注入 agent scope；
        ...
        // danger-full-access 会话（P、D(execute)）write/edit/bash 菜单去毒
        ...
        if (task.assignee === 'pt' || task.assignee === 'dt') {
          ...
          // local KB 模式：DT 改用 KB 写护栏（唯一可写区域=本地库根，评审页经 fs 写库根），
        ...
        // D(execute) 注入 git 凭据（repo-local http extraheader，GitLab glpat-* 用 oauth2 basic）。
        ...
        if (isDExecute && dRepo) {
          ...
          if (pat) {
            ...
            if (cred.ok) console.error('[dsh-swarm][debug] git credential injected task=' + taskId + ' targets=' + cred.detail);
            else console.error('[dsh-swarm][debug] git credential inject skipped task=' + taskId + ' repo=' + dRepo + ': ' + cred.detail);
          }
        }
      };
      try {
        // claim 后任何异常（buildContext/spawn）→ failTask（attempts+1）→ 调度器重派/看门狗熔断；
      ...
      // 本轮增量事件基线——agent 拿到之后、followup 之前记录。
      ...
      try {
        ...
        // 网关合成拒答兜底（2026-09-22）：jz 网关语义缓存（from-cache）/ 安全护栏（from-security-guard）
        // 会对首轮任务 prompt 合成 usage 全 0 的拒答，角色会话拿拒答当结论空闲退出
        // （protocol_violation 源头之一，销服一体 W2 卡会话 5 例实证）。检测到即自动
        // followup「继续未完成的任务」重试一次（限一次，防护栏连续拦截打穿重派链）；
        // 重试后再拦则照旧走终态判据（running → protocol_violation 显形）。
        // 读取容错对齐 session-events.ts 约定（snapshotEvents → events → []）；
        // seq > eventsBase 过滤本轮增量（防 resume 时历史 from-cache 标记误触发；fake/宿主
        // 变体无 seq 时 Number→NaN，比较恒 false → 检测跳过，不误伤）。
        const sessionLike = agent.session as unknown as {
          seq?: number; snapshotEvents?(fromSeq?: number): Array<Record<string, unknown>>; events?: Array<Record<string, unknown>>;
        };
        const turnEvents = typeof sessionLike.snapshotEvents === 'function'
        ...
        // 失败语义（统一）：发 failed 事件（attempts 递增），由调度器重派或看门狗熔断；不直接 block。
