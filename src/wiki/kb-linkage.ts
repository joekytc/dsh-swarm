...
function linkLine(label: string, pagePath: string): string {
...
function upsertLinkedBlock(content: string, header: string, lines: string[]): string {
...
export async function syncKbLinks(wiki: WikiVaultClient, state: BoardState, taskId: string): Promise<void> {
  ...
  if (checklistPath) {
    for (const p of pages) {
      ...
      if (containsLine(cur, line)) continue;
