 ...
 */
const PRESET_IDS = ['kanban-v', 'kanban-p', 'kanban-w', 'kanban-d', 'kanban-pt', 'kanban-dt', 'swarm'];
/** 包内组合目录（随包分发）。src/roles/ 与 lib/roles/ 深度一致，均经 ../../ 回到包根。 */
export function packagePresetsDir() {
...
export function installRolePresets() {
    ...
    for (const id of PRESET_IDS) {
        ...
        try {
            ...
            if (existsSync(meta))
                copyFileSync(meta, join(dstDir, 'preset.yml'));
            installed.push(id);
        }
        catch (err) {
