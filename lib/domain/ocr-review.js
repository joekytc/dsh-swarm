...
/** 解析失败的兜底结果。 */
const UNKNOWN_PREVIEW = { mode: 'unknown', files: [], excluded: [], mergeBase: null };
function isRecord(v) {
...
export function parseManagedJson(stdout) {
    let obj;
    try {
        obj = JSON.parse(stdout);
    }
...
/** 文件数严格超过阈值时建议托管评审。 */
export function shouldSuggestManaged(fileCount) {
...
export function reviewPagePath(repoSlug, topic, ymd) {
...
export function isStandaloneReviewNamespacePath(pagePath) {
