...
export class ConfigProvider extends Service {
    ...
    // mode 派生自 effective.wikiVault.baseUrl；运行期改 baseUrl 需重启 dsh 使客户端注册面同步（不可热切换）：
    ...
    diffKeys(prev, next) {
        ...
        if (prev.reviewEngine?.mode !== next.reviewEngine?.mode)
            keys.add('reviewEngine.mode');
        for (const f of ['provider', 'model']) {
            if (prev.reviewEngine?.managed?.[f] !== next.reviewEngine?.managed?.[f])
                keys.add('reviewEngine.managed.' + f);
        }
        if ((prev.imDelivery?.fallbackBotId ?? '') !== (next.imDelivery?.fallbackBotId ?? ''))
            keys.add('imDelivery.fallbackBotId');
        return [...keys];
    }
}
