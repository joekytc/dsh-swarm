...
export interface SourceEntry { type: 'TAPD' | 'Jira' | '其他'; url: string; note: string }
export interface PrdCollectionEntry {
  url: string;
  platform?: string; // feishu | wecom | modao | 其他（域名路由或 LLM 判定）
  status: 'collected' | 'blocked';
  summary: string; // 采集摘要：采了什么 + 关键信息 + 缺口
  pages?: string[]; // PRD 原文切片 KB 页路径
  blockedReason?: string; // status=blocked 必填：登录态 | 缺技能 | 404 | 其他
  degraded?: boolean; // 截图 >5MB 或写入失败 → 降级留痕
}
export interface PlaceholderEntry { target: string; value: string; replace: string }
...
export function validatePlanningChecklist(raw: unknown): string[] {
  ...
  if (!Array.isArray(clar)) {
    ...
    clar.forEach((el, i) => {
      ...
      if (ok) {
        const answer = (o['a'] as string).toLowerCase();
  ...
  if (!Array.isArray(sources) || sources.length === 0) {
    ...
    sources.forEach((el, i) => {
      ...
      if ((o['url'] as string).trim() === '' && (o['note'] as string).trim() === '') {
  ...
  if (!Array.isArray(prd)) {
    ...
    prd.forEach((el, i) => {
      const o = el as Record<string, unknown> | null;
      const urlOk = typeof o?.['url'] === 'string' && (o['url'] as string).trim().length > 0;
      ...
      if (typeof o['summary'] !== 'string' || (o['summary'] as string).trim() === '') {
        errors.push(`checklist.prdCollection[${i}].summary must be a non-empty string（采了什么+关键信息+缺口）`);
      }
      if (status === 'blocked' && (typeof o['blockedReason'] !== 'string' || (o['blockedReason'] as string).trim() === '')) {
    ...
    if (!Array.isArray(phs)) {
      ...
      phs.forEach((el, i) => {
        ...
        const ok = typeof o === 'object' && o !== null &&
          typeof o['target'] === 'string' && (o['target'] as string).trim() !== '' &&
          typeof o['value'] === 'string' &&
          typeof o['replace'] === 'string' && (o['replace'] as string).trim() !== '';
        if (!ok) {
          const got = typeof o === 'object' && o !== null ? Object.keys(o).join(',') : JSON.stringify(el);
...
export function formatChecklistBody(c: PlanningChecklist): string {
  ...
  if (prd.length === 0) {
    ...
    for (const p of prd) {
      ...
      if (p.status === 'blocked' && p.blockedReason) lines.push(`  - 阻塞原因: ${p.blockedReason}`);
      for (const pg of p.pages ?? []) lines.push(`  - 原文页: ${pg}`);
    }
  }
  lines.push('', '## 占位策略', '');
  const phs = c.placeholders ?? [];
  if (phs.length === 0) {
    ...
    for (const p of phs) lines.push(`| ${p.target} | ${p.value} | ${p.replace} |`);
  }
...
export function routePrdPlatform(url: string): string | null {
  let host: string;
  try {
  ...
  if (host === 'doc.weixin.qq.com') return 'wecom';
  if (host === 'modao.cc' || host.endsWith('.modao.cc')) return 'modao';
  return null;
}

/** PRD 原文切片：优先按一级标题（# 开头）切章，单片超 maxBytes 按行硬切。
 *  默认 45KB/片（wiki 大文档拆页惯例，低于服务端 body 上限留余量）。 */
export function slicePrdMarkdown(md: string, maxBytes = 45_000): string[] {
  ...
  if (cur.length > 0) chapters.push(cur.join('\n'));
  const parts: string[] = [];
