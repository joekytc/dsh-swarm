...
export function startDispatcher(ctx, config) {
...
function startDispatcherInner(ctx, config, storageDir, logFile, provider, agents) {
    ...
    const vOrch = new VOrchestrator(ctx, kanban, agents, config, orchestrations, wiki, defaultModel);
    ...
    ctx.on('dispose', () => { dispatcher.stop(); watchdog.stop(); });
