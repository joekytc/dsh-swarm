...
export { Config };
// 无 inject——KanbanProvider 只用文件系统（FileEventStore），不依赖 ctx.storage 等任何服务。
...
export function apply(ctx, config) {
    ...
    // ConfigProvider 在 apply 中创建一次（override 存放于 storageDir，$DSH_HOME 占位符在此解析），
    ...
    ctx.on('dispose', disposeImDelivery);
    ...
    // - 主会话工具面（spec_card_view/edit/approve + kanban 只读子集 + 前缀路由工具）
    //   收 ConfigProvider，wiki 客户端与全部配置读点经 getEffective() 调用时热读取。
    wireAllAvailable(ctx, ['tools', 'kanban'], () => registerMainSessionTools(ctx, configProvider));
    // - 调度层：事件唤醒 V + 每任务 agent runner + 看门狗（仅 agents 可用时启动）
    // startDispatcher 收 ConfigProvider（模型链/wiki 热生效）。
