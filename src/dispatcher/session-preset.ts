// src/dispatcher/session-preset.ts
/**
...
export function headerPresetOf(agent: unknown): string {
  const a = agent as PresetAgentLike | null | undefined;
  return asPresetId(a?.session?.header?.agentPreset);
}

/** 当前会话/agent 的 preset id（引擎真相 → 会话投影 → 创建 header → 默认 id → 空串）。 */
export function sessionPresetOf(ctx: PresetCtxLike | null | undefined, agent: unknown): string {
  const get = typeof ctx?.get === 'function' ? ctx.get.bind(ctx) : undefined;
  const presets = get?.('agentPresets') as
    { composedPreset?: (agentCtx: unknown) => unknown; defaultId?: unknown } | undefined;
  const a = agent as PresetAgentLike | null | undefined;
  // ① 引擎真相：读取失败（作用域链异常）不阻断调用方，降级到会话口径。
  if (a?.ctx !== undefined && typeof presets?.composedPreset === 'function') {
    let composed: unknown;
    try { composed = presets.composedPreset(a.ctx); } catch { composed = undefined; }
    const id = asPresetId(composed);
    if (id) return id;
  }
  // ② 会话日志口径。
  const projections = get?.('sessionProjections') as
    { stateOf?: (session: unknown, key: string) => unknown } | undefined;
  if (a?.session !== undefined && typeof projections?.stateOf === 'function') {
    let projected: unknown;
    try { projected = projections.stateOf(a.session, 'agentPreset'); } catch { projected = undefined; }
    const id = asPresetId(projected);
    if (id) return id;
  }
  // ③ 创建事实。
  const fromHeader = headerPresetOf(a);
  if (fromHeader) return fromHeader;
  // ④ 默认 id（读不到 → 空串 = 未知模式）。
  return asPresetId(presets?.defaultId);
}
