...
export function buildSubagentTreeGuard(deps = {}) {
    ...
    return (execution) => {
        ...
        if (!header || readPreset(execution?.agent) !== 'kanban-dt')
            return undefined;
...
export function buildSwarmSessionGuard(deps = {}) {
    const readPreset = deps.readPreset ?? headerPresetOf;
    return (execution) => {
        const header = extractSessionHeader(execution?.agent);
        if (!header || readPreset(execution?.agent) !== 'swarm')
            return undefined;
        const name = String(execution?.name ?? '');
        const args = execution?.arguments ?? {};
...
export const DEFAULT_WIKI_WRITE_PRESETS = ['swarm', 'kanban-w', 'ptc'];
export function buildStandaloneDtGuard(deps = {}) {
    const readPreset = deps.readPreset ?? headerPresetOf;
    return (execution) => {
        const header = extractSessionHeader(execution?.agent);
        const name = String(execution?.name ?? '');
        ...
        if (name === 'wiki_write') {
            if (isRoleComposed(execution?.agent) || isDtChainSubagent)
                return undefined;
            // 放宽（2026-09-16）：swarm 主会话与 W preset 会话可写 wiki——can('wiki-write') 本就含 w
            // （permissions.ts），写面由 wiki_write 工具内 assertAllowedWikiPagePath 六类命名空间
