...
import { createBotProbe, probeDepsFromCtx } from './im-bot-probe.js';
...
/** 归一：去空白（含全角空格）+ 小写（GUI 可能回传带装饰/大小写变化的文本）。 */
function normAnswer(s) {
    return s.replace(/[\s\u3000]/g, '').toLowerCase();
}
/** 一个候选的全部可识别令牌：botId / 展示 label 全文 / label 的展示名段（去 ·模式·离线 与 （botId）后缀）。 */
function idTokens(o) {
    const out = [normAnswer(o.botId)];
    if (o.label) {
        const n = normAnswer(o.label);
        out.push(n.split('（')[0], n.split('·')[0]);
    }
...
export function matchBotAnswer(options, raw) {
    const target = normAnswer(raw);
    if (!target)
        return '';
    const exact = options.find((o) => idTokens(o).includes(target));
    if (exact)
        return exact.botId;
    const partial = options.filter((o) => idTokens(o).some((t) => t.length >= 6 && (t.includes(target) || target.includes(t))));
    const uniq = [...new Set(partial.map((o) => o.botId))];
    return uniq.length === 1 ? uniq[0] : '';
}
/** 多机器人且无法自动决策时的 fail-closed 报错：列候选（有探针数据则带模式与连接态）。 */
function botCandidatesError(ids, probed) {
...
export async function chooseBot(im, cfg, deps) {
    const explicit = cfg.botId.trim();
    if (explicit)
        return { ok: true, botId: explicit, via: 'explicit' };
    ...
    if (ids.length === 1)
        return { ok: true, botId: ids[0], via: 'single' };
    const fallback = (cfg.fallbackBotId ?? '').trim();
    const preset = (deps.sessionPreset ?? '').trim();
    // 只有「能模式匹配」或「能交互」时才值得探针：纯自动路径 + 无默认 → 直接给候选报错，省一次握手。
    const probed = (preset || deps.ask) ? await deps.probe() : { ok: false, error: 'probe-skipped' };
    if (!probed.ok && (preset || deps.ask))
        deps.log(`[im-delivery] bot probe unavailable: ${probed.error}`);
    ...
    if (preset && candidates.length) {
        ...
        if (hit.length === 1)
            return { ok: true, botId: hit[0].botId, via: 'preset' };
    ...
    if (fallback && ids.includes(fallback))
        return { ok: true, botId: fallback, via: 'fallback' };
    if (fallback)
        deps.log(`[im-delivery] 默认机器人 ${fallback} 不在接入列表，转交互`);
    if (!deps.ask)
        return { ok: false, error: botCandidatesError(ids, candidates) };
    const options = ids.map((id) => {
        const c = candidates.find((b) => b.botId === id);
        if (!c)
            return { botId: id, label: '' };
        return { botId: id, label: `${c.label || '未命名'}·${c.agentPreset || '未设模式'}${c.connected ? '' : '·离线'}` };
    });
    const answer = await deps.ask(options);
    if (!answer)
        return { ok: false, error: '未选择投递机器人（交互取消或超时），本次不投递' };
    if (!answer.setDefault || !deps.setFallback)
        return { ok: true, botId: answer.botId, via: 'interactive' };
    const saved = await deps.setFallback(answer.botId);
    if (!saved)
        deps.log('[im-delivery] 默认机器人写入失败（配置未更新），本次仍按所选机器人投递');
...
async function readTargets(im, botId) {
    const raw = await im.listTargets(botId);
    if (Array.isArray(raw))
        return raw;
    const enveloped = raw?.targets;
    return Array.isArray(enveloped) ? enveloped : [];
}
...
export async function resolveTargetId(im, botId, botIdExplicit, cfg, kind) {
...
function buildBotAsk(ctx, agent, log) {
    ...
    return async (options) => {
        ...
        try {
            const ans = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error('im-bot-ask-timeout')), ASK_TIMEOUT_MS)),
            ]);
            const pick = ans.answers?.find((a) => a.id === 'im-bot');
            const typed = pick?.custom?.trim() ?? '';
            const selected = pick?.selected?.[0]?.trim() ?? '';
            // 选中标签与自由文本都接受，但必须唯一落到候选之一——防手写错 id 投到未知对象。
            const botId = matchBotAnswer(options, selected) || matchBotAnswer(options, typed);
            if (!botId) {
                // 只记候选 id 与用户可见文本（公开标识，无敏感载荷），便于下次定位映射失败。
                log(`[im-delivery] 机器人交互已应答但未映射到候选: selected=${JSON.stringify(selected).slice(0, 120)} custom=${JSON.stringify(typed).slice(0, 120)} candidates=${options.map((o) => o.botId).join(',')}`);
                return null;
            }
...
function buildSetFallback(configProvider) {
    return async (botId) => {
        try {
...
export function createSender(ctx, kanban, configProvider, opts = {}) {
    ...
    const probe = opts.botChoice?.probe ?? createBotProbe(probeDepsFromCtx(ctx));
...
export function wireImDelivery(ctx, kanban, configProvider, opts = {}) {
    ...
    const handle = (ev) => {
        void (async () => {
            ...
            if (!((ev.kind === 'task/completed' && ev.taskId) || ev.kind === 'chain/blocked' || ev.kind === 'review/gave-up'))
                return;
            const state = await kanban.snapshot();
